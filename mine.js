...
var siteName = document.querySelector("[name='site_name']"); // select from name type
var url = document.querySelector("[name='url']"); // select from name type
...
button.addEventListener("click", function(e){
    ...
    if(siteName.value === ""){
        ...
        arrayItems = localStorage.bookmark.split(";"); // .split == return array 
        adr = url.value;
        adr = adr.replace(/http:\/\/|https:\/\//i, "");
        arrayItems.length--; //remove the last element in array == (;)
            
        // Check if website is already bookmarked
        for(item of arrayItems){
            itemAdr = item.split(',')[1].replace(/http:\/\/|https:\/\//i,""); //take url from  split ==> item.split(',')[1]
            if(itemAdr == adr){
            check = true;
            }
        }
...
function removeBookmark(thisItem){
    let arrayItems = [],
        ...
        itemURL = item.querySelector(".visit").dataset.link, // to arrive the link in a tag in href
